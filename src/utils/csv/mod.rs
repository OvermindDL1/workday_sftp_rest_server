...
use crate::utils::csv::verifier::Verifier;
...
pub fn get_last_matching_file_delete_rest(globsb: &str, keep: usize) -> anyhow::Result<PathBuf> {
	...
	if paths.len() > keep {
		info_span!("Too many files matching `{globsb}`, deleting old:").in_scope(|| -> anyhow::Result<()> {
...
pub fn validate_csv<T: serde::Serialize>(data: &[T]) -> anyhow::Result<()> {
...
#[instrument(level = "debug", skip(data))]
pub fn write_csv_with_date_to<T: serde::Serialize>(base_path: &str, data: &[T], keep: usize) -> anyhow::Result<()> {
